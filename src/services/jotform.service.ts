...
export interface ParsedSubmission {
  ...
  totalAmount?: number;
  chooseYour?: string; // Stadium 25 field for Friday/Saturday selection
}
...
const parseProductDetails = (productField: any): { quantity: number; productDetails: string; totalAmount: number } => {
  ...
  try {
    ...
    logger.warn('parseProductDetails: Error parsing product details', { error, productField });
...
const parseFormFields = (fields: any): ParsedSubmission => {
  ...
  logger.info('Parsed chooseYour field', { 
